...
function PopupContent({ onClose, message }) {
  return (
    <>
...
export default function SubscribePopup({ isOpen, onClose, message }) {
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="hidden lg:flex fixed inset-0 bg-black/70 z-[100] items-center justify-center p-4 backdrop-blur-sm"
            onClick={onClose}
          >
            <motion.div
            ...
            >
              <PopupContent onClose={onClose} message={message} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Mobile bottom sheet */}
      <BottomSheet isOpen={isOpen} onClose={onClose} title="Credits Exhausted">
        <PopupContent onClose={onClose} message={message} />
      </BottomSheet>
    </>
  );
